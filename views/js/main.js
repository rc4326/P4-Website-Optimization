...
//replaced all the querySelectors with getElementsByClassName for faster times
var resizePizzas = function(size) {
    ...
    //update - removed unnecessary fucntion created variable to hold the new width



    // Changes the slider value to a percent width
    //update - minimized to 1 function
    //update - updated to be able to append a % sign in the changePizzaSizes function
...
var pizzasDiv = document.getElementById("randomPizzas"); //pulled the pizzasDiv out of the For Loop
for (var i = 2; i < 100; i++) {
...
// update - pulled the phase element out of the loop and saved it to a variable to call inside the loop in essence caching it.
//update - calculated fixed number of movers outside of the loop with the variable numItems to save reiterations
function updatePositions() {
    frame++;
    window.performance.mark("mark_start_frame");

    var items = document.getElementsByClassName('mover');
    var numItems = items.length; //counted the number of movers and assigned to variable pulling it out of the loop
    var cachedScrollTop = document.body.scrollTop; //scrollTop already runs when the function runs so no need to put it in loop
    var phase;
    for (var i = 0; i < numItems; i++) { //using the number of movers inside the loop
        phase = Math.sin((cachedScrollTop / 1250) + (i % 5)); //used the cached variable for the math calculation
...
//created loadedPizzas variable to dynamically adjust to the correct number of pizzas to display based on screen height and the height of the pizza image
//First figure out rows by dividing screen height by image pixels (100px) then multiply by number of columns (cols = 8)
document.addEventListener('DOMContentLoaded', function() {
    var cols = 8;
    var s = 256;
    var elem;
    var loadedPizzas = screen.height / 100 * cols; //calculate the needed images based upon the screen height
    var movingPizzas = document.getElementById("movingPizzas1"); //created a variable for the DOM element and pulled it outside the loop
    for (var i = 0; i < loadedPizzas; i++) {
        ...
        movingPizzas.appendChild(elem); //this is the call to attach movingPizzas1 to the DOM element mover
    }
    updatePositions();
});
